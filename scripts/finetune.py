...
def main() -> None:
    ...
    resume = bool(finetune_cfg.get("resume", False))

    # 先从本地 YOLO 权重构建完整检测器，再覆盖蒸馏阶段更新过的学生参数。
    model = YOLO(str(yolo_weights))
    if not resume:
        checkpoint_path = require_file(output_dir / "distill" / "best.pt", "Distillation checkpoint")
        ...
        print("Resuming Ultralytics finetune run; skip loading distill checkpoint manually.")
