...
def load_resume_checkpoint(
    ...
    projection: ProjectionHead,
    optimizer: torch.optim.Optimizer,
    student_ema: ModelEMA | None,
) -> tuple[int, float]:
    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    student.model.load_state_dict(checkpoint["yolo_state_dict"], strict=False)
    projection.load_state_dict(checkpoint["projection_state_dict"], strict=False)
    optimizer.load_state_dict(checkpoint["optimizer_state_dict"])
    if student_ema is not None and "yolo_ema_state_dict" in checkpoint:
        student_ema.ema.load_state_dict(checkpoint["yolo_ema_state_dict"], strict=False)
    start_epoch = int(checkpoint.get("epoch", 0)) + 1
    best_loss = float(checkpoint.get("loss", float("inf")))
    print(f"Resumed pretrain from {checkpoint_path} at epoch {start_epoch}.")
...
def main() -> None:
    ...
    if bool(train_cfg.get("resume", False)):
        resume_checkpoint = require_file(
            resolve_path(project_root, train_cfg.get("resume_checkpoint", "outputs/pretrain/last.pt")),
            "Pretrain resume checkpoint",
        )
        start_epoch, best_loss = load_resume_checkpoint(resume_checkpoint, student, projection, optimizer, student_ema)
